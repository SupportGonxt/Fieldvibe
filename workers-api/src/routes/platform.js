...
import { authMiddleware, requireRole, requireSuperAdmin } from '../lib/middleware.js';
...
app.post('/admin/payments/backfill-ledger', requireRole('admin', 'super_admin'), async (c) => {
  const db = c.env.DB;
...
app.post('/notifications', requireRole('admin', 'manager'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.post('/notifications/broadcast', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
// ==================== CROSS-TENANT ====================
app.get('/cross-tenant/assignments', requireRole('admin'), async (c) => {
  const db = c.env.DB;
...
app.post('/cross-tenant/assignments', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const body = await c.req.json();
  const id = uuidv4();
  const userId = c.get('userId');
...
app.delete('/cross-tenant/assignments/:id', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
...
app.post('/cross-tenant/switch', authMiddleware, async (c) => {
  const db = c.env.DB;
  const userId = c.get('userId');
  const body = await c.req.json();
...
// ==================== AUDIT LOG ====================
app.get('/audit-log', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.get('/settings-categories', async (c) => {
...
app.post('/settings/initialize', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const defaults = [
...
app.put('/settings', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
app.get('/individuals', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.get('/individuals/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.post('/individuals', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
app.put('/individuals/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  const body = await c.req.json();
...
app.get('/process-flows', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  try {
...
app.get('/process-flows/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  try {
...
// CREATE process flow
app.post('/process-flows', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
// UPDATE process flow
app.put('/process-flows/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  const body = await c.req.json();
...
app.delete('/process-flows/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
...
app.get('/company-process-flows', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.post('/company-process-flows', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
app.delete('/company-process-flows/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
...
app.get('/company-custom-questions', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.post('/company-custom-questions', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
app.put('/company-custom-questions/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
  const body = await c.req.json();
...
app.delete('/company-custom-questions/:id', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const id = c.req.param('id');
...
app.post('/migrations/create-process-flows', authMiddleware, async (c) => {
  const db = c.env.DB;
  const results = [];
  try {
...
app.post('/migrations/clear-stellr-company-questions', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  try {
...
app.post('/migrations/add-performance-indexes', authMiddleware, async (c) => {
  const db = c.env.DB;
  const results = [];
  try {
...
app.put('/feature-flags/:name', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.get('/process/audit', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.get('/tenants', requireRole('admin'), async (c) => {
  const db = c.env.DB;
...
app.get('/platform/tenants', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const role = c.get('role');
  const tenantId = c.get('tenantId');
  let tenants;
...
app.post('/tenants', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const body = await c.req.json();
...
app.post('/platform/tenants', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const body = await c.req.json();
  const tenantId = uuidv4();
  const code = body.code || body.name.toUpperCase().replace(/\s+/g, '_').substring(0, 20);
...
app.put('/tenants/:id', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
  const body = await c.req.json();
...
app.put('/platform/tenants/:id', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
  const body = await c.req.json();
  await db.prepare('UPDATE tenants SET name = COALESCE(?, name), domain = COALESCE(?, domain), subscription_plan = COALESCE(?, subscription_plan), max_users = COALESCE(?, max_users), updated_at = datetime("now") WHERE id = ?').bind(body.name || null, body.domain || null, body.subscriptionPlan || null, body.maxUsers || null, id).run();
...
app.delete('/tenants/:id', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
...
app.post('/tenants/:id/activate', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
...
app.post('/tenants/:id/suspend', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
...
app.get('/tenants/:id/users', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
...
app.get('/tenants/:id/modules', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
...
app.put('/tenants/:id/modules', requireSuperAdmin, async (c) => {
  const db = c.env.DB;
  const { id } = c.req.param();
  const body = await c.req.json();
...
app.get('/settings/company', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.put('/settings/company', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
app.get('/settings/modules', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.get('/platform/settings', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const tenant = await db.prepare('SELECT * FROM tenants WHERE id = ?').bind(tenantId).first();
  return c.json({ success: true, data: {
...
app.put('/platform/settings', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
...
// T.2 Webhooks
app.get('/webhooks', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.post('/webhooks', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const body = await c.req.json();
  const id = uuidv4();
...
app.put('/webhooks/:id', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const { id } = c.req.param();
  const body = await c.req.json();
...
app.delete('/webhooks/:id', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const { id } = c.req.param();
...
app.get('/webhooks/:id/deliveries', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const { id } = c.req.param();
...
// W.1 Error Logs
app.get('/error-logs', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
// X.1 Seed Demo Data
app.post('/seed/demo', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const userId = c.get('userId');
  const seedId = uuidv4();
...
app.get('/seed/runs', requireRole('admin'), async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.post('/seed/goldrush', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  const userId = c.get('userId');
  try {
...
app.post('/seed/goldrush-sample-board', authMiddleware, async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  try {
...
app.get('/brand-owner/dashboard', async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
...
app.get('/brand-owner/reports', async (c) => {
  const db = c.env.DB;
  const tenantId = c.get('tenantId');
  let brandId = c.req.query('brand_id') || '';
  if (!brandId) {
...
// Workflow routes
app.get('/workflow/processes', authMiddleware, async (c) => {
  return c.json({ success: true, data: {
...
app.get('/workflow/documentation', authMiddleware, async (c) => {
  return c.json({ success: true, data: {
...
app.get('/events/analytics/summary', authMiddleware, async (c) => {
  try {
    const db = c.env.DB;
    const tenantId = c.get('tenantId');
...
// ai routes
app.get('/ai/chat/comprehensive-analysis', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/config', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/customers/:customerId/fraud-check', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/customers/:customerId/insights', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/field-agents/:id/insights', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/orders/:orderId/fraud-check', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/orders/insights', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/ai/chat/products/:productId/insights', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
...
// audit routes
app.get('/audit/:id/:subId', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/audit/:id/:subId/entries/:entryId', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
...
// documents routes
app.get('/documents/:documentId/:documentId/relationships', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/documents/:documentId/:documentId/relationships/:relationshipId', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/documents/relationships', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});
app.get('/documents/relationships/:relationshipId', authMiddleware, async (c) => {
  try { const tenantId = c.get('tenantId'); return c.json({ success: true, data: [], total: 0 }); }
  catch (e) { return c.json({ success: false, message: e.message }, 500); }
});

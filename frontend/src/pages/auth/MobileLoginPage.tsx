...
const MobileLoginPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      if (role === 'general_manager') { navigate('/agent/overview'); return }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        navigate(authUser?.role === 'backoffice_admin' ? '/agent/reconcile' : authUser?.role === 'general_manager' ? '/agent/overview' : '/agent/dashboard')
      } else {
  ...
  const handlePinChangeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        navigate(loggedInRole === 'backoffice_admin' ? '/agent/reconcile' : loggedInRole === 'general_manager' ? '/agent/overview' : '/agent/dashboard')
      } else {
  ...
  if (mustChangePin) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-surface via-[#0F2140] to-[#162D50] flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 relative z-10">
          ...
          <div className={`w-full max-w-sm ${shake ? 'animate-[shake_0.5s_ease-in-out]' : ''}`}>
            <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-token p-6 shadow-2xl">
              {pinChangeError && (
              ...
              <form onSubmit={handlePinChangeSubmit} className="space-y-5">
                {/* New PIN */}
                <div>
                  <label className="block text-xs font-medium text-token-muted mb-2 uppercase tracking-wider">New PIN</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-token-faint" />
                    <input
                      ...
                      autoFocus
                      className="w-full pl-11 pr-12 py-3.5 bg-white/5 border border-token rounded-xl text-token placeholder-gray-500 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all text-base tracking-[0.3em]"
                    />
                    <button
                      type="button"
                      onClick={() => setShowNewPin(!showNewPin)}
                      className="absolute right-3.5 top-1/2 -translate-y-1/2 text-token-faint hover:text-token-muted transition-colors"
                    >
                      {showNewPin ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                    </button>
                  </div>
                  <p className="text-xs text-token-faint mt-1.5">4-6 digits, must be different from 12345</p>
                </div>

                {/* Confirm PIN */}
                <div>
                  <label className="block text-xs font-medium text-token-muted mb-2 uppercase tracking-wider">Confirm PIN</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-token-faint" />
                    <input
                      type={showNewPin ? 'text' : 'password'}
                      ...
                      className="w-full pl-11 pr-4 py-3.5 bg-white/5 border border-token rounded-xl text-token placeholder-gray-500 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all text-base tracking-[0.3em]"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-primary to-[#00D06E] text-on-primary font-semibold rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/30 disabled:opacity-40 disabled:shadow-none disabled:cursor-not-allowed transition-all active:scale-[0.98] text-base"
                >
                  {pinChangeLoading ? (
        ...
        <div className="pb-6 text-center relative z-10">
          <p className="text-xs text-gray-600">A Product of <span className="text-token-faint">GONXT</span></p>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 relative z-10">
        ...
        <div className="mb-8 text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-primary to-[#00B86B] rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-primary/20">
            <span className="text-2xl font-black text-on-primary">FV</span>
          </div>
          <h1 className="text-2xl font-bold text-token tracking-tight">FieldVibe</h1>
          <p className="text-sm text-token-muted mt-1">Agent Login</p>
        </div>
        ...
        <div className={`w-full max-w-sm ${shake ? 'animate-[shake_0.5s_ease-in-out]' : ''}`}>
          <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-token p-6 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Phone */}
              <div>
                <label className="block text-xs font-medium text-token-muted mb-2 uppercase tracking-wider">Phone Number</label>
                <div className="relative">
                  <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-token-faint" />
                  <input
                    type="tel"
                    value={phone}
                    ...
                    autoFocus
                    autoComplete="tel"
                    className="w-full pl-11 pr-4 py-3.5 bg-white/5 border border-token rounded-xl text-token placeholder-gray-500 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all text-base"
                  />
                </div>
              </div>
              ...
              {/* PIN */}
              <div>
                <label className="block text-xs font-medium text-token-muted mb-2 uppercase tracking-wider">PIN Code</label>
                <div className="relative">
                  <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-token-faint" />
                  <input
                    ref={pinRef}
                ...
                <p className="text-xs text-token-faint mt-1.5">4-6 digit PIN from your manager</p>
              </div>
          ...
          <div className="mt-6 text-center">
            <button
              ...
              className="text-sm text-token-faint hover:text-primary transition-colors"
            >
              Admin / Manager Login
            </button>
          </div>

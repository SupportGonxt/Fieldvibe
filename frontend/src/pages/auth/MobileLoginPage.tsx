...
const MobileLoginPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      if (role === 'general_manager') { navigate('/agent/pnl'); return }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        navigate(authUser?.role === 'backoffice_admin' ? '/agent/reconcile' : authUser?.role === 'general_manager' ? '/agent/pnl' : '/agent/dashboard')
      } else {
  ...
  const handlePinChangeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        navigate(loggedInRole === 'backoffice_admin' ? '/agent/reconcile' : loggedInRole === 'general_manager' ? '/agent/pnl' : '/agent/dashboard')
      } else {

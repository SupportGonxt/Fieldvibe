...
function statusIcon(status: string) {
  switch (status) {
    ...
    default: return <Clock className="w-4 h-4 text-token-muted" />
  }
}
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-token-muted text-sm">Agent not found</p>
  ...
  return (
    <div className="min-h-screen bg-bg pb-24">
      ...
      <div className="bg-surface px-5 py-4 border-b border-token">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-token-muted mb-3 min-h-[44px] -ml-2 px-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-token">{agent.first_name} {agent.last_name}</h1>
            <p className="text-xs text-token-faint">{agent.phone || agent.role}</p>
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] text-token-faint">Achievement</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-white/5 border border-token rounded-xl p-3">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <p className="text-[10px] text-token-faint uppercase">Today Individual</p>
            </div>
            <p className="text-xl font-bold text-token">{stats.today_visits}</p>
          </div>
          <div className="bg-white/5 border border-token rounded-xl p-3">
            <div className="flex items-center gap-1.5 mb-1">
              <Store className="w-3 h-3 text-purple-400" />
              <p className="text-[10px] text-token-faint uppercase">Today Store</p>
            </div>
            <p className="text-xl font-bold text-token">{stats.today_stores}</p>
          </div>
          <div className="bg-white/5 border border-token rounded-xl p-3">
            <div className="flex items-center gap-1.5 mb-1">
              <MapPin className="w-3 h-3 text-emerald-400" />
              <p className="text-[10px] text-token-faint uppercase">Month Individual</p>
            </div>
            <p className="text-xl font-bold text-token">{stats.month_visits}</p>
          </div>
          <div className="bg-white/5 border border-token rounded-xl p-3">
            <div className="flex items-center gap-1.5 mb-1">
              <Store className="w-3 h-3 text-amber-400" />
              <p className="text-[10px] text-token-faint uppercase">Month Store</p>
            </div>
            <p className="text-xl font-bold text-token">{stats.month_stores}</p>
          </div>
        </div>

        {/* Target Progress */}
        <div className="bg-white/5 border border-token rounded-xl p-4 mb-4">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-token font-medium">{stats.actual_visits}/{stats.target_visits} <span className={pctClass(vPct)}>({vPct}%)</span></span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-token font-medium">{stats.actual_stores}/{stats.target_stores} <span className={pctClass(rPct)}>({rPct}%)</span></span>
              </div>
              <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
        ...
        <div>
          ...
          {visibleVisits.length === 0 ? (
            <div className="bg-white/5 border border-token rounded-xl p-6 text-center">
              ...
              <p className="text-sm text-token-faint">{filterParam === 'rejected_photos' ? 'No rejected photo visits' : 'No visits found'}</p>
            </div>
          ) : (
              ...
              {visibleVisits.map((visit) => {
                ...
                return (
                  <button
                    ...
                    className="w-full bg-white/5 border border-token rounded-xl p-3 flex items-center gap-3 text-left active:bg-white/10 transition-colors"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-token truncate">{displayName}</p>
                      {hasRejected && (
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] text-token-faint flex items-center gap-0.5">

...
import { Download, AlertTriangle, RefreshCw, XCircle, Users } from 'lucide-react'
...
interface UploadFailure {
  ...
  errors: { id_number?: string; goldrush_id?: string }
...
const GoldrushUploadFailuresReport: React.FC = () => {
  ...
  const uniqueTeamLeads = new Set(failures.map(f => f.team_lead_id).filter(Boolean)).size
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const headers = ['Date', 'Team Lead', 'Agent', 'First Name', 'Last Name', 'ID Number', 'Goldrush ID', 'ID Number Error', 'Goldrush ID Error']
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Captures rejected due to invalid SA ID number or Goldrush ID format</p>
        </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-100 dark:border-gray-700/50">
                          <th className="text-left py-2 px-4 text-xs text-gray-500 dark:text-gray-400 font-medium">Date</th>
                          <th className="text-left py-2 px-4 text-xs text-gray-500 dark:text-gray-400 font-medium">Name</th>
                          <th className="text-left py-2 px-4 text-xs text-gray-500 dark:text-gray-400 font-medium">ID Number</th>
                          <th className="text-left py-2 px-4 text-xs text-gray-500 dark:text-gray-400 font-medium">Goldrush ID</th>
                          <th className="text-left py-2 px-4 text-xs text-gray-500 dark:text-gray-400 font-medium">Reason Not Loaded</th>
                        </tr>
                      </thead>
                      <tbody>
                        {agentGroup.records.map(record => (
                          <tr key={record.visit_id} className="border-b border-gray-50 dark:border-gray-700/30 hover:bg-red-50/50 dark:hover:bg-red-900/10">
                            ...
                            <td className="py-2.5 px-4 text-gray-900 dark:text-white font-medium whitespace-nowrap">
                            ...
                            <td className="py-2.5 px-4">
                              ...
                              <p className="text-xs text-gray-400 mt-0.5 max-w-[240px]">{record.error_summary}</p>
                            </td>
                          </tr>
                        ))}
                      </tbody>
      ...
      {failures.length > 0 && (
        <div className="text-sm text-gray-500 dark:text-gray-400">
          Showing {failures.length} failed capture{failures.length !== 1 ? 's' : ''} across {uniqueAgents} agent{uniqueAgents !== 1 ? 's' : ''} and {uniqueTeamLeads} team lead{uniqueTeamLeads !== 1 ? 's' : ''}
        </div>
      )}
    </div>
  )
